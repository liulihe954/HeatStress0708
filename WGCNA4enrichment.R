...
#================================================================================================
#########################################
###  filter the data as you prefered ####
#########################################
# options:
# 1. variance
# 2. low counts
# 3. less 0 counts in each group
# 4. ...
#########################################

### notes "datExpr14_cl"  is referece and "datExpr14_ht" is test dataset
### format: sample * genes (sample in rows and GENEid in columns)
#================================================================================================
...
softPower_b = min(which(sft_b_cl$fitIndices[,2] > 0.8))
...
MEList14_bicor_cl = moduleEigengenes(datExpr14_bicor_cl,colors = dynamicColors14_bicor_cl)
MEs14_bicor_cl = removeGreyME(MEList14_bicor_cl$eigengenes)
# Calculate dissimilarity of module eigengenes
MEDiss14_bicor_cl = 1 - cor(MEs14_bicor_cl)
# Cluster module eigengenes
METree14_bicor_cl = hclust(as.dist(MEDiss14_bicor_cl), method = "average");
# Plot the result of module eigengenes
sizeGrWindow(8,16)
pdf("Clustering of module eigengenes bicor 14_cl.pdf",height=8,width=16)
plot(METree14_bicor_cl, main = "Clustering of module eigengenes bicor 14_cl",
     xlab = "", sub = "")
...
pdf("all_module_preservation_statistics_nested bicor.pdf",height = 8, width = 16)
par(mfrow = c(4,5))
...
nonpres_modulenames_b = rownames(Z.PreservationStats14_b)[nonpres_index]
KEGG_results_b = list()
pdf("KEGG Enrichment in modules bicor.pdf")
for (i in c(1:length(nonpres_modulenames_b))){
  #i = 3
  module_name = nonpres_modulenames_b[i]
  nopresID = as.vector(colnames(datExpr14_b_cl)[which(moduleColors14_b_cl == module_name)])
...
total.genes = colnames(datExpr14_b_cl) # total genes in your dataset
...
for (i in c(1:(length(nonpres_modulenames_b)))){
  module_name = nonpres_modulenames_b[i]
  nopresID_GO = as.vector(colnames(datExpr14_b_cl)[which(moduleColors14_b_cl == module_name)])
