...
save(networkData14_normalized_normfactors,networkData14_normalized,file = "networkData84 norm and factors.RData")
...
datExpr14_cl = t(networkData14_filnal[,names(networkData14_filnal) %in% names(networkData)[column_84_cl] ])
datExpr14_ht = t(networkData14_filnal[,names(networkData14_filnal) %in% names(networkData)[column_84_ht] ])
...
# plot samples
pdf(file = "Plots_sampleClustering_day84.pdf", width = 12, height = 9)
sizeGrWindow(12,9)
par(mfrow = c(1,2))
par(cex = 0.6)
plot(sample_Tree_ht, main = "Sample clustering to detect outliers ht84 group",
     sub="", xlab="", cex.lab = 1.5, cex.axis = 1.5, cex.main = 2)
plot(sample_Tree_cl, main = "Sample clustering to detect outliers cl84 group",
     sub="", xlab="", cex.lab = 1.5, cex.axis = 1.5, cex.main = 2)
dev.off()
...
sizeGrWindow(9,5)
cex1 = 0.9
pdf(file = "soft_threshold_picking_day84.pdf", width = 12, height = 9)
par(mfrow = c(1,2))
# Scale-free topology fit index as a function of the soft-thresholding power
plot(sft_cl$fitIndices[,1], -sign(sft_cl$fitIndices[,3])*sft_cl$fitIndices[,2],
     xlab="Soft Threshold (power) 84_cl",ylab="Scale Free Topology Model Fit, unsigned R^2 84_cl",type="n",
     main = paste("Scale independence 84_cl"));
...
plot(sft_cl$fitIndices[,1], sft_cl$fitIndices[,5],
     xlab="Soft Threshold (power) 84_cl",ylab="Mean Connectivity 84_cl",type="n",
     main = paste("Mean connectivity 84_cl"))
text(sft_cl$fitIndices[,1], sft_cl$fitIndices[,5], labels=powers,cex=cex1,col="red")
abline(h=MeanK,col="red")
dev.off()
#save
save(sft_cl,softPower,MeanK,file = "SoftThres_day84.RData")
...
# save the matrix
save(adjacency14_cl,dissTOM14_cl,geneTree14_cl,adjacency14_ht,dissTOM14_ht,geneTree14_ht,file = "AllMatrixday84.RData")
...
#==========================================================================================
# cor
pdf("dendrogram84.pdf",     height=6,width=16)
par(mfrow=c(1,2))
plot(geneTree14_cl,xlab="",sub="",main="Gene clustering on TOM-based dissimilarity (84_cl)",labels=FALSE,hang=0.04);
plot(geneTree14_ht,xlab="",sub="",main="Gene clustering on TOM-based dissimilarity (84_ht)",labels=FALSE,hang=0.04);
dev.off()
...
sizeGrWindow(8,16)
pdf("dendrogram_84cl_nomg.pdf",height=8,width=16)
plotDendroAndColors(geneTree14_cl, dynamicColors14_cl, "Dynamic Tree Cut 84_cl nomg",
                    dendroLabels = FALSE, hang = 0.03,
                    addGuide = TRUE, guideHang = 0.05,
                    main = "Gene dendrogram and module colors 84_cl nomg")
dev.off()
...
sizeGrWindow(8,16)
pdf("Clustering_of_module_eigengenes _84_cl.pdf",height=8,width=16)
plot(METree14_cl, main = "Clustering of module eigengenes 84_cl",
     xlab = "", sub = "")
...
#                              6. plotting heatmap                                            ###
#=================================================================================================
pdf("Network_heatmap_plot_all_genes_84_cl.pdf",height=8,width=16)
for (i in c(6:12)){
...
write.csv(Obs.PreservationStats14,"Obs.PreservationStats_day84.csv")
write.csv(Z.PreservationStats14,"Z.PreservationStats_day84.csv")
...
#
pdf("medianRank_Zsummary_versus_module_size_day84.pdf",height = 8, width = 16)
...
plot(moduleSize14[selectModules],medianRank14[selectModules],col=1,
     bg=modColors14[selectModules], pch = 21,main="medianRank Preservation day84",
...
plot(moduleSize14[selectModules],Zsummary14[selectModules], col = 1,
     bg=modColors14[selectModules],pch = 21,main="Zsummary Preservation day84",
...
pdf("all_module_preservation_statistics_nested_day84.pdf",height = 8, width = 16)
par(mfrow = c(4,5))
...
KEGG_results = list()
pdf("KEGG_Enrichment_in_modules_day84.pdf")
...
dev.off()
save(KEGG_results, file = "KEGG_results_day84.RData")
...
save(GO_results, file = "GO_results_day84.RData")
